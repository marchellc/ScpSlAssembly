...
namespace PlayerRoles.PlayableScps.Scp079.GUI
{
	public class Scp079AuxGui : Scp079BarBaseGui
	{
		protected override string Text
		{
			get
			{
				return string.Format("{0} / {1}", this._auxManager.CurrentAuxFloored, this._auxManager.MaxAux);
			}
		}
		...
		protected override float FillAmount
		{
			get
			{
				return this._auxManager.CurrentAux / this._auxManager.MaxAux;
			}
		}
		...
		private string BuildHeader(Scp079HudTranslation header)
		{
			...
			this._sb.Append(">");
			this._sb.Append(Translations.Get<Scp079HudTranslation>(header));
			this._sb.Append("</size>");
		...
		protected override void Update()
		{
			...
			{
				IScp079AuxRegenModifier scp079AuxRegenModifier = allSubroutines[i] as IScp079AuxRegenModifier;
				if (scp079AuxRegenModifier != null)
				{
					float auxRegenMultiplier = scp079AuxRegenModifier.AuxRegenMultiplier;
					if (auxRegenMultiplier < 1f)
					{
						flag = true;
