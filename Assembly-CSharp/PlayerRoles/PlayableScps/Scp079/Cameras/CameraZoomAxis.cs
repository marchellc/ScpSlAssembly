...
namespace PlayerRoles.PlayableScps.Scp079.Cameras
{
	...
	public class CameraZoomAxis : CameraAxisBase
	{
		public float CurrentZoom
		{
			get
			{
				return this._magnificationCurve.Evaluate(base.CurValue);
			}
		}
		...
		{
			float num = Mathf.InverseLerp(base.MinValue, base.MaxValue, newValue);
			this._zoomBone.localPosition = Vector3.Lerp(this._unzoomedOffset.position, this._zoomedOffset.position, num);
			this._zoomBone.localRotation = Quaternion.Lerp(Quaternion.Euler(this._unzoomedOffset.rotation), Quaternion.Euler(this._zoomedOffset.rotation), num);
			this._zoomBone.localScale = Vector3.Lerp(this._unzoomedOffset.scale, this._zoomedOffset.scale, num);
			if (this.SoundEffectSource.loop)
			{
				return;
			}
			if (this._lastSoundZoom == base.TargetValue)
			{
				return;
			}
		...
		private void UpdateInputs()
		{
			...
			{
				base.TargetValue += ((axisRaw > 0f) ? this._stepSize : (-this._stepSize));
