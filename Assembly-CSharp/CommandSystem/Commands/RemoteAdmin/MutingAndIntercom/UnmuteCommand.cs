	...
	public class UnmuteCommand : ICommand, IUsageProvider
	{
		...
		public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
		{
			...
			if (list != null)
			{
				...
				{
					PlayerUnmutingEventArgs playerUnmutingEventArgs = new PlayerUnmutingEventArgs(referenceHub2, referenceHub, false);
					PlayerEvents.OnUnmuting(playerUnmutingEventArgs);
					if (playerUnmutingEventArgs.IsAllowed)
					{
						VoiceChatMutes.RevokeLocalMute(referenceHub2.authManager.UserId, false);
						PlayerEvents.OnUnmuted(new PlayerUnmutedEventArgs(referenceHub2, referenceHub, false));
						ServerLogs.AddLog(ServerLogs.Modules.Administrative, sender.LogName + " unmuted player " + referenceHub2.LoggedNameFromRefHub() + ".", ServerLogs.ServerLogType.RemoteAdminActivity_GameChanging, false);
						num++;
					}
				}
			}
