	...
	public class IntercomMuteCommand : ICommand, IUsageProvider
	{
		...
		public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
		{
			...
			if (list != null)
			{
				...
				{
					PlayerMutingEventArgs playerMutingEventArgs = new PlayerMutingEventArgs(referenceHub2, referenceHub, true);
					PlayerEvents.OnMuting(playerMutingEventArgs);
					if (playerMutingEventArgs.IsAllowed)
					{
						VoiceChatMutes.IssueLocalMute(referenceHub2.authManager.UserId, true);
						PlayerEvents.OnMuted(new PlayerMutedEventArgs(referenceHub2, referenceHub, true));
						ServerLogs.AddLog(ServerLogs.Modules.Administrative, sender.LogName + " issued an intercom mute to player " + referenceHub2.LoggedNameFromRefHub() + ".", ServerLogs.ServerLogType.RemoteAdminActivity_GameChanging, false);
						num++;
					}
				}
			}
