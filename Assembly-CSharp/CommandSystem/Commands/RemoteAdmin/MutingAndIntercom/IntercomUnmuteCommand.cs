	...
	public class IntercomUnmuteCommand : ICommand, IUsageProvider
	{
		...
		public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
		{
			...
			if (list != null)
			{
				...
				{
					PlayerUnmutingEventArgs playerUnmutingEventArgs = new PlayerUnmutingEventArgs(referenceHub2, referenceHub, true);
					PlayerEvents.OnUnmuting(playerUnmutingEventArgs);
					if (playerUnmutingEventArgs.IsAllowed)
					{
						VoiceChatMutes.RevokeLocalMute(referenceHub2.authManager.UserId, true);
						PlayerEvents.OnUnmuted(new PlayerUnmutedEventArgs(referenceHub2, referenceHub, true));
						ServerLogs.AddLog(ServerLogs.Modules.Administrative, sender.LogName + " revoked an intercom mute of player " + referenceHub2.LoggedNameFromRefHub() + ".", ServerLogs.ServerLogType.RemoteAdminActivity_GameChanging, false);
						num++;
					}
				}
			}
