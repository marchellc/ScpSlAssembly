...
namespace CommandSystem.Commands.Shared
{
	...
	public class ValueCommand : ICommand, IUsageProvider
	{
		...
		public string[] Aliases { get; } = new string[] { "val" };

		public string Description { get; } = "Returns the value of specified config key";

		public string[] Usage { get; } = new string[] { "Key" };
		...
		public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
		{
			...
			string text;
			if (ConfigFile.ServerConfig.TryGetString(arguments.At(0), out text))
			{
				response = "The value of <i>'" + arguments.At(0) + "'</i> is: " + text;
				return true;
			}
