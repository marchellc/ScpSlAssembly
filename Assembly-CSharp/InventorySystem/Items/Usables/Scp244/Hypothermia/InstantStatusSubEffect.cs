...
namespace InventorySystem.Items.Usables.Scp244.Hypothermia
{
	public class InstantStatusSubEffect : HypothermiaSubEffectBase, IWeaponModifierPlayerEffect, ISearchTimeModifier, IMovementSpeedModifier
	{
		private float CurIntensity
		{
			get
			{
				return this._currentIntensity * RainbowTaste.CurrentMultiplier(base.Hub);
			}
		}
		...
		private float VitalityMultiplier
		{
			get
			{
				return (float)(Vitality.CheckPlayer(base.Hub) ? 0 : 1);
			}
		}
		...
		public override bool IsActive
		{
			get
			{
				return this._currentIntensity > 0f;
			}
		}
		...
		public float MovementSpeedMultiplier
		{
			get
			{
				return Mathf.LerpUnclamped(1f, this._movementSpeedMultiplier, this.VitalityMultiplier * this.CurIntensity);
			}
		}
		...
		internal override void UpdateEffect(float curExposure)
		{
			...
			{
				foreach (AttachmentParameterValuePair attachmentParameterValuePair in this._weaponStats)
				{
