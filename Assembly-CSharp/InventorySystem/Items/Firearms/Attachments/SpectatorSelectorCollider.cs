...
namespace InventorySystem.Items.Firearms.Attachments
{
	public class SpectatorSelectorCollider : MonoBehaviour, IAttachmentSelectorButton
	{
		...
		public void Setup(Texture icon, AttachmentSlot slot, Vector2? pos, Firearm fa)
		{
			if (icon != null)
			{
				Vector2 vector = new Vector2((float)icon.width, (float)icon.height);
				this._firearm = fa;
				this._mySlot = slot;
				this._image.texture = icon;
				this._image.rectTransform.sizeDelta = vector;
				if (pos != null)
				{
		...
		public void Hover(bool isHovering)
		{
			if (isHovering)
			{
				this._selector.ShowStats((int)this.ButtonId);
				return;
			}
		...
		public void UpdateColors(AttachmentSlot slot)
		{
			...
			bool flag = slot == this._mySlot || (this._firearm != null && (int)this.ButtonId < this._firearm.Attachments.Length && this._firearm.Attachments[(int)this.ButtonId].IsEnabled);
