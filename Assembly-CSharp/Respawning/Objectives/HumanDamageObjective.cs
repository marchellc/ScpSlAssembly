...
public class HumanDamageObjective : HumanObjectiveBase<DamageObjectiveFootprint>
{
	...
	private void OnPlayerDamaged(ReferenceHub victim, DamageHandlerBase dhb)
	{
		...
		if (this.IsValidFaction(faction) && victim.playerStats.TryGetModule<HealthStat>(out var module))
		{
			if (!HumanDamageObjective.HealthTracker.TryGetValue(victim, out var value))
			{
				value = 0;
			}
			int num = Mathf.FloorToInt((1f - module.NormalizedValue) * 100f / 10f);
			if (num < HumanDamageObjective.MaxThresholdAmount && num > value)
			{
				HumanDamageObjective.HealthTracker[victim] = num;
				int num2 = num - value;
				float num3 = -2f * (float)num2;
				float num4 = 1f * (float)num2;
				base.ReduceTimer(faction, num3);
				base.GrantInfluence(faction, num4);
