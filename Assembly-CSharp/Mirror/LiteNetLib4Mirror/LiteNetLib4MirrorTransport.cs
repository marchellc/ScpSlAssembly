...
namespace Mirror.LiteNetLib4Mirror
{
	public class LiteNetLib4MirrorTransport : Transport
	{
		...
		{
			if (LiteNetLib4MirrorCore.Host.ConnectedPeersCount >= (int)this.maxConnections)
			{
				request.Reject();
				return;
			}
			if (request.AcceptIfKey(LiteNetLib4MirrorServer.Code) == null)
			{
		...
		{
			return LiteNetLib4MirrorUtils.ToBase64(string.Concat(new string[]
			{
				Application.productName,
				Application.companyName,
				Application.unityVersion,
				"1.2.8",
				LiteNetLib4MirrorTransport.Singleton.authCode
			}));
		}
		...
		{
			byte b = (byte)((channelId < this.channels.Length) ? channelId : 0);
			LiteNetLib4MirrorClient.Send(this.channels[(int)b], data.Array, data.Offset, data.Count, b);
		}
		...
		{
			return new Uri(string.Format("{0}:{1}", this.serverIPv4BindAddress, this.port));
		}
		...
		public bool ServerSend(List<int> connectionIds, int channelId, ArraySegment<byte> data)
		{
			byte b = (byte)((channelId < this.channels.Length) ? channelId : 0);
			bool flag = true;
			foreach (int num in connectionIds)
			{
				flag &= LiteNetLib4MirrorServer.Send(num, this.channels[0], data.Array, data.Offset, data.Count, b);
			}
			return flag;
		}
		...
		{
			byte b = (byte)((channelId < this.channels.Length) ? channelId : 0);
			LiteNetLib4MirrorServer.Send(connectionId, this.channels[(int)b], data.Array, data.Offset, data.Count, b);
		}
		...
		public DeliveryMethod[] channels = new DeliveryMethod[]
		{
