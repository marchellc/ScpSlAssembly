...
namespace ServerOutput
{
	public class TcpConsole : IServerOutput, IDisposable
	{
		public int ReceiveBufferSize
		{
			get
			{
				return this._client.ReceiveBufferSize;
			}
		}
		...
		public int SendBufferSize
		{
			get
			{
				return this._client.SendBufferSize;
			}
		}
		...
		public TcpConsole(ushort port, int receiveBufferSize = 25000, int sendBufferSize = 200000)
		{
			...
			this._queueThread = new Thread(new ThreadStart(this.Send))
			{
				Priority = global::System.Threading.ThreadPriority.Lowest,
				IsBackground = true,
		...
		public void Start()
		{
			this._client.Connect(new IPEndPoint(IPAddress.Loopback, (int)this._port));
		...
		private unsafe void Receive()
		{
			...
			while (!this._disposing)
			{
				try
				{
					...
					if (num != 4)
					{
						ServerConsole.AddLog(string.Format("[TcpConsole] Received header length is NOT {0}! Received data amount: {1}.", 4, num), ConsoleColor.Red, false);
					}
					else
					{
						...
						if (num != num2)
						{
							ServerConsole.AddLog(string.Format("[TcpConsole] Received data length is NOT {0}! Received data amount: {1}.", num2, num), ConsoleColor.Red, false);
						}
						else
						{
			...
			while (!this._disposing)
			{
				...
				else
				{
					try
					{
						...
						if (this._prompterQueue.TryDequeue(out outputEntry))
						{
							byte[] array = ArrayPool<byte>.Shared.Rent(outputEntry.GetBytesLength());
							int num;
							outputEntry.GetBytes(ref array, out num);
