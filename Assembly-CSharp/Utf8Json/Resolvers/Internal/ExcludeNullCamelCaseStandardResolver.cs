...
internal sealed class ExcludeNullCamelCaseStandardResolver : IJsonFormatterResolver
{
	private static class FormatterCache<T>
	{
		...
		static FormatterCache()
		{
			...
			{
				FormatterCache<T>.formatter = (IJsonFormatter<T>)ExcludeNullCamelCaseStandardResolver.fallbackFormatter;
			}
			else
			{
	...
	private sealed class InnerResolver : IJsonFormatterResolver
	{
		...
		private static readonly IJsonFormatterResolver[] resolvers = StandardResolverHelper.CompositeResolverBase.Concat(new IJsonFormatterResolver[1] { DynamicObjectResolver.ExcludeNullCamelCase }).ToArray();
