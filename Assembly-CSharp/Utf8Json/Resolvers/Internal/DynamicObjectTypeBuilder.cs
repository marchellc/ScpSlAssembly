...
internal static class DynamicObjectTypeBuilder
{
	...
	internal static class EmitInfo
	{
		...
		public static readonly MethodInfo GetFormatterWithVerify = typeof(JsonFormatterResolverExtensions).GetRuntimeMethod("GetFormatterWithVerify", new Type[1] { typeof(IJsonFormatterResolver) });
	...
	public static object BuildAnonymousFormatter(Type type, Func<string, string> nameMutator, bool excludeNull, bool allowPrivate, bool isException)
	{
		...
		if (isException)
		{
			HashSet<string> ignoreSet = new HashSet<string>(new string[6] { "HelpLink", "TargetSite", "HResult", "Data", "ClassName", "InnerException" }.Select((string x) => nameMutator(x)));
	...
	private static LocalBuilder EmitNewObject(ILGenerator il, Type type, MetaType info, DeserializeInfo[] members, bool isSideEffectFreeType)
	{
		if (info.IsClass)
		{
			...
			if (info.BestmatchConstructor != null)
			{
				...
				{
					il.EmitLdloc(members.First((DeserializeInfo x) => x.MemberInfo == item).LocalField);
				}
		...
		else
		{
			...
			{
				il.EmitLdloc(members.First((DeserializeInfo x) => x.MemberInfo == item2).LocalField);
			}
			il.Emit(OpCodes.Newobj, info.BestmatchConstructor);
